...
type UserRestAPIHandler struct {
	logger                  *pkg.Logger
	userRegistrationUseCase *userapplication.RegistrationUseCase
	userLoginUseCase        *userapplication.LoginUserUseCase
	userRetrieveUseCase     *userapplication.RetrieveUserUseCase
	userRetrieveAllUseCase  *userapplication.RetrieveAllUsersUseCase
	userUpdateUseCase       *userapplication.UpdateUserUseCase
	userDeleteUseCase       *userapplication.DeleteUserUseCase
	authMiddleware          *middleware.AuthMiddleware
}

func NewUserRestAPIHandler(
	logger *pkg.Logger,
	useCase *userapplication.UserUseCase,
...
func (h *UserRestAPIHandler) UserRegistration(c *gin.Context) {
	...
	output, err := h.userRegistrationUseCase.Execute(c.Request.Context(), &userapplication.RegistrationInput{
		Email:    req.Email,
...
func (h *UserRestAPIHandler) UserLogin(c *gin.Context) {
	...
	output, err := h.userLoginUseCase.Execute(c.Request.Context(), &userapplication.LoginInput{
		Username: req.Username,
		Password: req.Password,
	})
...
func (h *UserRestAPIHandler) UserUpdate(c *gin.Context) {
	...
	if err := h.userUpdateUseCase.Execute(c.Request.Context(), &userapplication.UpdateInput{
		Email:    req.Email,
		Username: req.Username,
		Bio:      req.Bio,
		Image:    &req.Image,
