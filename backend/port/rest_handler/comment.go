...
type CommentRestAPIHandler struct {
	logger                    *pkg.Logger
	commentCreateUseCase      *commentapplication.CreateCommentUseCase
	commentRetrieveUseCase    *commentapplication.RetrieveCommentUseCase
	commentRetrieveAllUseCase *commentapplication.RetrieveAllCommentUseCase
	commentUpdateUseCase      *commentapplication.UpdateCommentUseCase
	commentDeleteUseCase      *commentapplication.DeleteCommentUseCase
	authMiddleware            *middleware.AuthMiddleware
}

func NewcommentRestAPIHandler(
	logger *pkg.Logger,
	commentUseCase *commentapplication.CommentUseCase,
...
func (h *CommentRestAPIHandler) CommentCreate(c *gin.Context) {
	...
	output, err := h.commentCreateUseCase.Execute(c.Request.Context(), &commentapplication.CreateCommentInput{
		Body:      req.Body,
		AuthorID:  authorID,
...
func (h *CommentRestAPIHandler) CommentUpdate(c *gin.Context) {
	...
	if err := h.commentUpdateUseCase.Execute(c.Request.Context(), &commentapplication.UpdateCommentInput{
		Body: req.Body,
	}, uint(id)); err != nil {
