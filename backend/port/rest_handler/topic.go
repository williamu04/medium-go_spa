...
type TopicRestAPIHandler struct {
	logger                  *pkg.Logger
	topicCreateUseCase      *topicapplication.CreateTopicUseCase
	topicRetrieveAllUseCase *topicapplication.RetrieveAllTopicUseCase
	topicUpdateUseCase      *topicapplication.UpdateTopicUseCase
	topicDeleteUseCase      *topicapplication.DeleteTopicUseCase
	authMiddleware          *middleware.AuthMiddleware
}

func NewtopicRestAPIHandler(
	logger *pkg.Logger,
	useCase *topicapplication.TopicUseCase,
...
func (h *TopicRestAPIHandler) TopicCreate(c *gin.Context) {
	...
	output, err := h.topicCreateUseCase.Execute(c.Request.Context(), &topicapplication.CreateTopicInput{
		Topic: req.Topic,
	})
...
func (h *TopicRestAPIHandler) TopicUpdate(c *gin.Context) {
	...
	if err := h.topicUpdateUseCase.Execute(c.Request.Context(), &topicapplication.UpdateTopicInput{
		Topic: req.Topic,
	}, uint(id)); err != nil {
