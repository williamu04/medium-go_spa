...
type ArticleRestAPIHandler struct {
	logger                    *pkg.Logger
	articleCreateUseCase      *article_application.CreateArticleUseCase
	articleRetrieveUseCase    *article_application.RetrieveArticleUseCase
	articleRetrieveAllUseCase *article_application.RetrieveAllArticleUseCase
	articleUpdateUseCase      *article_application.UpdateArticleUseCase
	articleDeleteUseCase      *article_application.DeleteArticleUseCase
	authMiddleware            *middleware.AuthMiddleware
}

func NewArticleRestAPIHandler(
	logger *pkg.Logger,
	useCase *article_application.ArticleUseCase,
...
func (h *ArticleRestAPIHandler) RegisterArticleRoutes(router *gin.RouterGroup) {
	...
	router.GET("/all", h.ArticleRetrieveAll)
...
func (h *ArticleRestAPIHandler) ArticleCreate(c *gin.Context) {
	...
	if !ok {
		h.logger.Error("article ID not found")
	...
	if !ok {
		h.logger.Error("Invalid article ID type")
		pkg.Error(c, http.StatusInternalServerError, "Internal server error")
		return
	}

	output, err := h.articleCreateUseCase.Execute(c.Request.Context(), &article_application.CreateArticleInput{
		Title:       req.Title,
...
func (h *ArticleRestAPIHandler) ArticleUpdate(c *gin.Context) {
	...
	if err := h.articleUpdateUseCase.Execute(c.Request.Context(), &article_application.UpdateArticleInput{
		Title:       req.Title,
		Description: req.Description,
		Body:        req.Body,
		Topic:       req.Topic,
